...
func Modemreset(comport string) (serial.Port,error) {
	...
	err = TestPort(comport)
	if  err != nil {
		return nil,errors.New("Modemreset: Testcomport #1 failed: " + err.Error())
	}
...
//	err=Modemcommand(port,"AT&F\r","OK",time.Second,"reset modem2",nil)
	err=Modemcommand(port,"ATZ\r","OK",time.Second*2,"reset modem2",err)
	err=Modemcommand(port,"ATE0; V1\r","OK",time.Second*2,"echo & verbose",err)
	err=Modemcommand(port,"AT+CSCA=\"0046708000708\"\r","OK",time.Second*2,"setup SMS CENTER",err)
//	err=Modemcommand(port,"AT+CSMP=17,167,0,16\r","OK",time.Millisecond*300,"Flash SMS ON",err)
	err=Modemcommand(port,"AT+CSMP=17,167,0,0\r","OK",time.Second*2,"Flash SMS OFF",err)
	err=Modemcommand(port,"AT+CMGF=0\r","OK",time.Second*2,"set PDU mode",err)
	...
	// err=Modemcommand(port,"AT+CGATT?\r","",time.Second*3,"check GATT",err)
	// err=Modemcommand(port,"AT+CFUN=0\r","",time.Second*3,"set full func off",err)
	// err=Modemcommand(port,"ATZ\r","",time.Second,"reset modem",err)
	// err=Modemcommand(port,"ATE0\r","",time.Second,"set echo on",err)
	// err=Modemcommand(port,"ATV1\r","",time.Second,"set verbose on",err)
	// err=Modemcommand(port,"AT\r","OK",time.Second,"test AT command",err)
	// err=Modemcommand(port,"AT+CFUN=1\r","",time.Second*2,"set full func on",err)
	// err=Modemcommand(port,"AT\r","OK",time.Second,"test AT command",err)
	...
	if err!=nil {
		port.Close()
		port=nil
		return nil,errors.New("Modemreset failed: " + err.Error())
	}
...
func SendDirectSMS(port serial.Port, phoneNumber string, message string) error {
	...
	for i := 0; i < len(cmd1); i++ {
		fmt.Println("Sending part ", i+1, " of ", len(cmd1))
...
func myread(port serial.Port,response string,timeout time.Duration) (string,error) {
	...
	for {
		...
		if time.Since(myreadtimestart) > myreadtimeout {
            ...
            log.Printf("myread timeout %dms, waiting for %s (got '%s').\r\n", time.Since(startTime).Milliseconds(),timeout-time.Since(startTime),r)
		}
		if time.Since(startTime) > timeout {
            return r,errors.New("myread timeout exceeded, no expected response: " + response+ " widthin "+timeout.String())
        }
	}
	return r,err
}
...
func TestPort(comport string) error {
	...
	err = Modemcommand(port, "AT\r", "", time.Second*2, "wakeup", err)
	err = Modemcommand(port, "AT\r", "OK", time.Second*2, "wakeup", err)
	// if n==0 {
	// 	port.Close()
